...
fn schema_v3() -> PathBuf {
    fixture("elemento-requisito-v3.toml")
}

/// Referência quebrada é erro de arquivo: reprova a execução como
/// qualquer outro defeito, sem tratamento especial no código de saída.
#[test]
fn referencia_quebrada_sai_com_codigo_um() {
    let saida = rodar(&[
        schema_v3().to_str().unwrap(),
        fixture("exemplo-v3-invalido.toml").to_str().unwrap(),
        "--idioma",
        "pt",
    ]);
    ...
    assert!(texto.contains("FALHA"));
    assert!(texto.contains("fluxos_alternativos[0].ancora"));
}

/// Defeito no schema (referência apontando para o que não é chave) não
/// pode reprovar um arquivo que está correto: sai como aviso, código 0.
#[test]
fn arquivo_v3_correto_sai_com_codigo_zero() {
    let saida = rodar(&[
        schema_v3().to_str().unwrap(),
    ...
    assert!(
        ...
        "esperava sucesso; stdout: {}",
        stdout(&saida)
    );
    let texto = stdout(&saida);
    assert!(texto.contains("campos reconhecidos"), "bloco em português");
    assert!(texto.contains("fields recognized"), "bloco em inglês");
}
